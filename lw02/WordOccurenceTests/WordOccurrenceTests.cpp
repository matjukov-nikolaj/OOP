...
BOOST_AUTO_TEST_SUITE(calculation_frequency_words_occurrence)
	...
	BOOST_AUTO_TEST_CASE(calculate_word_occurrence_where_special_characters_are_connected_to_word)
	{
		WordsOccurrence result = CalculationFrequencyWordsOccurrence(".word, word, word. (char) \"char\" 'char'");
		WordsOccurrence exprectedResult = {
			{ "char", 3 },
			{ "word", 3 }
		};
	...
	BOOST_AUTO_TEST_CASE(try_to_calculate_word_occurrence_in_string_without_words)
	{
		WordsOccurrence result = CalculationFrequencyWordsOccurrence("., ., ., ., ,, ,, ... () ()() [][] []");
		BOOST_CHECK(result.empty());
	}
